...
import { Search, TrendingUp, Clock, ArrowRight, Beaker } from 'lucide-react'
...
const StockSearch = () => {
  ...
  const normalizeSymbol = (rawSymbol) => {
    if (!rawSymbol) return ''
    // Remove exchange suffixes like ":NASDAQ", ":NYSE" etc.
    const withoutExchange = String(rawSymbol).split(':')[0]
    return withoutExchange.toUpperCase()
  }
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const mapped = (results.data || []).map((r) => ({
        // Ensure consistent fields for UI and selection
        symbol: normalizeSymbol(r.symbol),
        rawSymbol: r.symbol,
        name: r.instrument_name || r.name || r.description || r.symbol,
        description: r.instrument_name || r.name || r.description || ''
      }))
  ...
  const handleStockSelect = async (stock) => {
    const safeSymbol = normalizeSymbol(stock.symbol)
    ...
    try {
      ...
      try {
        profileData = await stockAPI.getProfile(safeSymbol)
      } catch (profileError) {
        console.log('Could not fetch profile for', safeSymbol)
      }
      ...
      dispatch({ 
        ...
        payload: { 
          symbol: safeSymbol, 
          name: profileData?.name || stock.name || stock.description || safeSymbol,
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-3">Analyze Any Stock</h1>
        <p className="text-lg text-gray-600">Real-time data, financials, news, and quant strategy testing</p>
      </div>
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="flex space-x-8">
          {[
            ...
            { id: 'strategies', label: 'Quant Strategies', icon: Beaker }
          ].map((tab) => (
            <button
              key={tab.id}
        ...
        <>
          ...
          <div className="card mb-8">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
              <TrendingUp className="w-5 h-5 mr-2" /> Popular Stocks
            </h2>
          ...
          {state.searchHistory.length > 0 && (
            <div className="card">
              <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
                <Clock className="w-5 h-5 mr-2" /> Recent Searches
              </h2>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {item.sector && (
                            <span className="text-xs bg-primary-100 text-primary-700 px-2 py-1 rounded">{item.sector}</span>
                          )}
                        </div>
                        <div className="text-sm font-medium text-gray-700 mb-1">{item.name}</div>
                        {item.industry && <div className="text-xs text-gray-500 mb-2">{item.industry}</div>}
                        {item.marketCap && (
                          <div className="text-xs text-gray-600">Market Cap: ${(parseFloat(item.marketCap) / 1e9).toFixed(2)}B</div>
                        )}
                      </div>
